/////////////////////////////////////////////////////////////////////////////
/// @file mediasmartserver_leds.ino
///
/// Arduino Pro Micro firmware for controlling HP MediaSmart Server LEDs
...
// Bay LED pins: 4 bays x 2 colors
const int PIN_BLUE[] = {2, 3, 4, 5};
const int PIN_RED[]  = {6, 7, 8, 9};
const int NUM_BAYS = 4;

// System LED pins
const int PIN_SYS_BLUE = 10;  // PWM-capable on Pro Micro
const int PIN_SYS_RED  = 16;

// USB indicator LED pin
const int PIN_USB = 14;
...
void updateBayLeds() {
    for (int i = 0; i < NUM_BAYS; i++) {
        writePin(PIN_BLUE[i], bayBlue[i]);
        writePin(PIN_RED[i], bayRed[i]);
    }
}
...
void updateSystemLeds() {
    ...
    writePin(PIN_SYS_RED, redVisible);
}
...
void setup() {
    ...
    Serial.begin(115200);
    while (!Serial) {
        ; // Wait for USB CDC to be ready (Pro Micro specific)
    }
}
